...
-   **User Authentication:** A simple login and sign-up system.
...
3.  **Open `index.html` in your web browser.**
    _For the best experience, it is recommended to use a live server extension (like "Live Server" in VS Code) to serve the files, which prevents potential issues with browser security policies._
...
2.  Navigate to the **Menu** page, add a few items to your cart using the **Quick Add** button.
3.  Open the cart sidebar, apply the promo code **TASTY10** for a 10% discount, and click **Checkout**.
4.  On the **Payment** page, select a payment method (e.g., Cash on Delivery) and click **Pay Now**.
5.  You will be redirected to the **Order Tracker** page, which initially shows the status as "Preparing".
...
3.  Find the order and change its status from "Preparing" to "Out for Delivery" using the dropdown, then click **Update**.
4.  Switch back to the customer's **Order Tracker** tab. The tracker will automatically refresh to show the new status.
